<h1 align="center">Nicolás Ángel Rojas Yáñez</h1>

<p align="center">
  <b>Estudiante de Ingeniería de Sistemas</b> · Corporación Universitaria Remington · Séptimo semestre<br>
  Los Patios, Norte de Santander, Colombia
</p>

<p align="center">
  <a href="mailto:nr039020@gmail.com"><img src="https://img.shields.io/badge/Correo-nr039020%40gmail.com-1f2a36?style=for-the-badge&logo=gmail&logoColor=white" alt="Correo"></a>
  <a href="https://postvental.com.co"><img src="https://img.shields.io/badge/Sitio-postvental.com.co-2c3743?style=for-the-badge&logo=googlechrome&logoColor=white" alt="Sitio web"></a>
  <a href="https://github.com/Nico1091/CERTIFICADOS"><img src="https://img.shields.io/badge/Certificaciones-10%20diplomas-3d4753?style=for-the-badge&logo=coursera&logoColor=white" alt="Certificaciones"></a>
  <img src="https://img.shields.io/badge/Disponible-para%20contrataci%C3%B3n-2f6f4e?style=for-the-badge" alt="Disponible para contratación">
</p>

---

Programo en **Python, JavaScript y Java**, diseño y consulto **bases de datos relacionales**, y construyo
**servicios web que integran modelos de lenguaje**. También trabajo **redes**: direccionamiento, VLAN, listas
de control de acceso y NAT sobre Cisco Packet Tracer.

Administro desde noviembre de 2024 una tienda en línea propia, [POSTVENTAL](https://postvental.com.co), de
extremo a extremo: catálogo, carrito, pasarela de pago, seguimiento de pedidos y atención al cliente
automatizada con un modelo de lenguaje, además del dominio, el alojamiento y la red.

**Busco un equipo donde asumir tareas de desarrollo, datos e integración.**

---

## Proyectos

### [agent-data-toolkit](https://github.com/Nico1091/Nico_dab) · Python

API de pago por llamada para agentes de IA. Cobra en **USDC sobre la red Base** mediante el protocolo
**x402**: seis puntos de acceso tarifados, control de límites de gasto, manifiesto de descubrimiento,
pruebas automatizadas e integración continua.

<p>
  <img src="https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white">
  <img src="https://img.shields.io/badge/FastAPI-009688?style=flat-square&logo=fastapi&logoColor=white">
  <img src="https://img.shields.io/badge/x402-1f2a36?style=flat-square">
  <img src="https://img.shields.io/badge/USDC%20%C2%B7%20Base-2775CA?style=flat-square">
</p>

### [ScannerLab](https://github.com/Nico1091/Scannerlab) · JavaScript

Escáner de red local de código abierto. Descubre dispositivos y puertos en la red, mide latencia y
velocidad, y lo presenta todo en una interfaz web propia.

<p>
  <img src="https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=nodedotjs&logoColor=white">
  <img src="https://img.shields.io/badge/Express-000000?style=flat-square&logo=express&logoColor=white">
  <img src="https://img.shields.io/badge/Redes-3d4753?style=flat-square">
</p>

### [Tutor de Ingeniería](https://github.com/Nico1091/Desarollo--gemma4-Hackhaton) · JavaScript

Tutor para estudiantes de Ingeniería de Sistemas apoyado en modelos de lenguaje **ejecutados en local**,
con búsqueda de recursos y apoyo de accesibilidad. Presentado al **Gemma 4 Good Hackathon** de Kaggle y
Google DeepMind, categoría *Future of Education*.

<p>
  <img src="https://img.shields.io/badge/Node.js-339933?style=flat-square&logo=nodedotjs&logoColor=white">
  <img src="https://img.shields.io/badge/LM%20Studio-4b2ee8?style=flat-square">
  <img src="https://img.shields.io/badge/Hackathon-c46210?style=flat-square&logo=kaggle&logoColor=white">
</p>

### [Panel OpenClaw](https://github.com/Nico1091/Openclaw_Database) · Python

Extracción automática de conversaciones a una base **SQLite** y panel web que las visualiza en gráficos
interactivos, con actualización periódica.

<p>
  <img src="https://img.shields.io/badge/Flask-000000?style=flat-square&logo=flask&logoColor=white">
  <img src="https://img.shields.io/badge/SQLite-003B57?style=flat-square&logo=sqlite&logoColor=white">
  <img src="https://img.shields.io/badge/Chart.js-FF6384?style=flat-square&logo=chartdotjs&logoColor=white">
</p>

### [Prácticas de Cisco](https://github.com/Nico1091/Ejemplos_ciscoPackettarcer) · Redes

Ocho topologías de red listas para abrir y estudiar en Packet Tracer: direccionamiento, VLAN, listas de
control de acceso y NAT.

<p>
  <img src="https://img.shields.io/badge/Cisco-1BA0D7?style=flat-square&logo=cisco&logoColor=white">
  <img src="https://img.shields.io/badge/Packet%20Tracer-005073?style=flat-square">
  <img src="https://img.shields.io/badge/CCNA-3d4753?style=flat-square">
</p>

### [Banco Unisimón](https://github.com/Nico1091/Proyectos) · SQL

Modelo conceptual, modelo lógico, script de creación e interfaz web de un sistema bancario académico.

<p>
  <img src="https://img.shields.io/badge/SQL-4479A1?style=flat-square&logo=mysql&logoColor=white">
  <img src="https://img.shields.io/badge/Modelado%20relacional-3d4753?style=flat-square">
</p>

---

## Herramientas

| | |
|---|---|
| **Lenguajes** | Python · JavaScript · Java · SQL · HTML y CSS |
| **Bases de datos** | MySQL · PostgreSQL · SQLite · Modelado relacional y normalización |
| **Redes** | TCP/IP y modelo OSI · Subredes · VLAN · Listas de control de acceso · NAT · Cisco Packet Tracer |
| **Entorno** | Git y GitHub · Visual Studio Code · Docker · Linux |
| **Modelos de lenguaje** | Consumo de APIs · Asistentes conectados a bases de datos · Ejecución en local |
| **Idiomas** | Español (lengua materna) · Inglés (A2) |

---

## Formación

**Ingeniería de Sistemas** — Corporación Universitaria Remington, Medellín (modalidad virtual)
Séptimo semestre · 108 de 160 créditos aprobados

**Ingeniería de Sistemas** — Universidad Simón Bolívar, Cúcuta
Estudios previos al traslado · Veinte asignaturas convalidadas

**Técnico en Asistencia Administrativa** — SENA, Regional Norte de Santander

### Certificaciones

Diez diplomas en ciberseguridad, programación y control de versiones, emitidos por **TryHackMe**,
**MinTIC**, **Platzi**, **SENA** y **Generación TIC**. Los documentos originales están publicados en
**[Nico1091/CERTIFICADOS](https://github.com/Nico1091/CERTIFICADOS)**; el de programación básica del
MinTIC es verificable en blockchain.

---

<p align="center">
  <a href="mailto:nr039020@gmail.com">nr039020@gmail.com</a> · 322 391 2699 ·
  <a href="https://postvental.com.co">postvental.com.co</a>
</p>
